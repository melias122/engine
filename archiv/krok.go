...
func statCifrovackyStrings(n, m, lenOrigHeader int, sc []*statCifrovacky, tmax func() []byte) [][]string {
	...
	for j, stat := range sc {
		for u := 0; u <= maxUdalost && u < int(tMax[j]); u++ {
			...
			s[i+5] = append(s[i+5], itoa(last))
			s[i+6] = append(s[i+6], itoa(ap+last))
			s[i+7] = append(s[i+7], itoa(hp+last))
			s[i+8] = append(s[i+8], itoa(vp+last))
		}
	}
...
func (a *Archiv) statistikaCifrovacky() error {
	...
	w := rw.NewCsvMaxWriter(a.WorkingDir, "KrokCifrovacky", [][]string{header})
	defer w.Close()
